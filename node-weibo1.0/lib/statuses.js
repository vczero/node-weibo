...
var statuses={
    ...
    tags:tags,
    tags_show_batch:tags_show_batch,
    tag_timeline_ids:tagTimeline_ids,
    tags_create:tags_create,
    tags_destroy:tags_destroy,
...
function tags(options,callback){
    var url=statuses.url+'tags.json';
    packurl.packGetUrl(url,options,callback);
}

//根据提供的微博ID批量获取此微博的标签信息
function tags_show_batch(options,callback){
    var url=statuses.url+'tags/show_batch.json';
    packurl.packGetUrl(url,options,callback);
}

//获取当前用户某个标签的微博ID列表
function tagTimeline_ids(options,callback){
    var url=statuses.url+'tag_timeline/ids.json';
    packurl.packGetUrl(url,options,callback);
}

//创建当前登陆用户微博的标签
function tags_create(options,callback){
    var url=statuses.url+'tags/create.json';
    packurl.packPostUrl(url,options,callback);
}

//删除当前登录用户微博的标签
function tags_destroy(options,callback){
    var url=statuses.url+'tags/destroy.json';
    packurl.packPostUrl(url,options,callback);
}

//更新当前登录用户微博的标签
function tags_update(options,callback){
    var url=statuses.url+'tags/update.json';
    packurl.packPostUrl(url,options,callback);
}

//更新当前登录用户某个微博的标签
function update_tags(options,callback){
    var url=statuses.url+'update_tags.json';
    packurl.packPostUrl(url,options,callback);
}
